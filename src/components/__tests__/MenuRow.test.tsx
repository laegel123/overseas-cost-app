...
describe('MenuRow', () => {
  ...
  // ─── disabled ─────────────────────────────────────────────────────────────
  describe('disabled', () => {
    it('disabled=true → opacity-50 + accessibilityState.disabled + onPress 미호출', () => {
      // Pressable 의 disabled prop 은 RN 내부에서 처리되므로 RNTL 의 host
      // node props 로 surface 되지 않음. 사용자 관점 동작 (onPress 차단 +
      // accessibilityState.disabled) 으로 충분히 검증.
  ...
  // ─── showChevron ──────────────────────────────────────────────────────────
  describe('chevron', () => {
    it('showChevron=true (default) → chev-right 아이콘 렌더 (testID-chevron)', () => {
      render(<MenuRow icon="settings" label="x" testID="r" />);
      expect(screen.getByTestId('r-chevron')).toBeTruthy();
    });

    it('showChevron=false → chevron 미렌더', () => {
      render(
        <MenuRow icon="info" label="앱 정보" variant="dim" showChevron={false} testID="r" />,
      );
      expect(screen.queryByTestId('r-chevron')).toBeNull();
    });
  });
