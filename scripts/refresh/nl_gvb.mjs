/**
 * scripts/refresh/nl_gvb.mjs
 *
 * GVB (Gemeentelijk Vervoerbedrijf Amsterdam) → 암스테르담 transport 갱신.
 *
 * 출처: GVB fare 공식 페이지
 * URL: https://en.gvb.nl/abonnementen
 *
 * 방법:
 * - monthlyPass: GVB 월정액 (Amsterdam region)
 * - singleRide: 1-hour ticket
 * - taxiBase: 암스테르담 택시 기본요금
 */
...
export const CITY_CONFIGS = {
  amsterdam: {
    ...
    region: 'eu',
    transitOperator: 'GVB',
    fareUrl: 'https://en.gvb.nl/abonnementen',
  },
};
...
export const SOURCE = {
  category: 'transport',
  name: 'GVB official fares + static estimates',
  url: 'https://en.gvb.nl/abonnementen',
};
...
/**
 * GVB fare 페이지 확인 (connectivity 체크용).
...
export default async function refresh(opts = {}) {
  ...
  if (!opts.useStatic) {
    const pageAvailable = await checkGvbFarePage();
    if (!pageAvailable) {
      errors.push({
        cityId: 'amsterdam',
        reason: 'GVB fare page unavailable, using static values',
      });
    }
  }
