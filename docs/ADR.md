...
4. **`ca_statcan` 식재료 8종 중 일부 항목 static fallback** (`onion1kg`, `apple1kg`, `ramen` 등):
   ...
   - **STATIC_PRICES 기준년도 / CPI 기준년도 일치 검증 필수**:
     - `cpiToPrice(cpi, basePrice) = (cpi/100) × basePrice` 변환식이 정확하려면 `basePrice` 가 CPI 기준년도의 가격이어야 함.
     - StatCan WDS Vector (Table 18-10-0004) 의 CPI 기준년도가 2002=100 인지 2020=100 인지 출처 확인 후 `STATIC_PRICES` 갱신.
     - 현재 `STATIC_PRICES` 는 2024~2026 시장가 기준 (provisional) — step 4 재개 / 운영 검증 시 정확도 재확인 + `sources[].name` 에 기준년도 명시.
