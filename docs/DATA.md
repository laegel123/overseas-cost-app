...
| 시드 데이터 (오프라인 fallback) | `data/seed/all.json` (앱 번들)                | 실 도시 데이터 21개 전량 — `build_data.mjs` 가 `all.json` 과 함께 생성 (ADR-074) | 동일                |
...
모든 데이터 포인트는 `sources` 배열에 카테고리별 출처 1개 이상 명시. 출처 없는 데이터는 추가하지 않는다.

사용자 노출 경로 (v1.0 실제 구현):
...
- [ ] 시드(`data/seed/all.json`)와 `data/all.json` 내용 일치 (ADR-074 — `build_data.mjs` 가 보장)
...
- 앱 자체의 사용자 개인정보 수집 0건 (계정·로그인 없음, 분석 도구 없음 — ADR-009/011). 광고 SDK(AdMob)가 수집하는 정보는 개인정보 처리방침 '광고' 섹션 (ADR-077)
...
- [ ] 시드(21개 전량)와 cities 디렉터리 동기화 (ADR-074)
