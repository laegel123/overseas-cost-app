...
### 9.8.1 `src/store/rentChoice.ts` (ADR-060)

월세 카테고리는 4 주거 형태 (share/studio/oneBed/twoBed) 합산이 의미 없어
(한 사람이 동시 거주 X) 사용자가 비교 기준으로 선택한 한 가지 형태를 단일
출처로 두는 store. Detail 화면에서 바꾼 선택이 Compare hero / 월세 카드에도
즉시 반영되도록 양 화면이 동일 store 구독.

**기본 동작:**

- [x] 초기 상태 `{ rentChoice: 'share' }`
- [x] `setRentChoice(...)` 4 literal 모두 반영 (share/studio/oneBed/twoBed)
- [x] `reset()` → 초기 상태 복귀

**영속화:**

- [x] persist key 정확히 `rentChoice:v1`
- [x] partialize: 액션 미영속화, rentChoice 만 저장
- [x] round-trip: storage 에 박힌 v1 entry → rehydrate 후 메모리 반영

**손상 캐시 / 알 수 없는 literal — INITIAL fallback (silent fail 금지):**

- [x] 잘못된 JSON → INITIAL
- [x] 알 수 없는 rentChoice literal (e.g., `'penthouse'`) → INITIAL
- [x] rentChoice 누락 → INITIAL
- [x] rentChoice 가 number 등 잘못된 타입 → INITIAL

**`RENT_CHOICE_FALLBACK_ORDER`:**

- [x] 순서 고정: `share` → `studio` → `oneBed` → `twoBed`

**`resolveRentChoice` (순수 함수 — Compare/Detail 단일 fallback 정책):**

- [x] 선택값이 있으면 그대로 반환 `{ key, value }`
- [x] 선택값이 null → fallback 순서로 첫 non-null
- [x] 선택값 + 일부 fallback 도 null → 다음 fallback 으로 진행
- [x] 모든 키가 null → null 반환 (호출자 분기)
- [x] 선택 키 자체는 fallback 순회에서 skip (중복 검사 방지)

**Hydration:**

- [x] `waitForAllStoresHydrated` 가 본 store 도 동시 await (`hydration.ts` 5 store)

**마이그레이션 (deferred):**

- [ ] (v2 도입 시 추가) v1 → v2: migrate 함수 spy + 새 필드 기본값 검증

### 9.8.2 `src/store/tuitionChoice.ts` (ADR-061)

학비 카테고리 단일 선택 + 직접 입력 store. 도시별 map (`Record<cityId, choice>`) —
도시별 학교 라인업이 다르므로 각 도시 선택값을 독립적으로 보존. preset (school
이름) 또는 custom (사용자 입력 연 학비). Detail 시트에서 바꾼 선택이 Compare
hero / 학비 카드에도 즉시 반영.

**기본 동작:**

- [x] 초기 상태 `{ choices: {} }` (도시별 미선택)
- [x] `setTuitionChoice(cityId, { kind: 'preset', school })` 도시별 entry
- [x] `setTuitionChoice(cityId, { kind: 'custom', annual })` custom entry
- [x] 도시별 선택은 독립 — paris / london 동시 보존
- [x] `clearTuitionChoice(cityId)` 그 도시만 제거 (다른 도시 선택 보존)
- [x] `clearTuitionChoice` 미존재 cityId → state 변경 없음 (no-op, 객체 동일성 유지)
- [x] `reset()` 모든 도시 선택 제거

**영속화:**

- [x] persist key 정확히 `tuitionChoice:v1`
- [x] partialize: 액션 미영속, choices 만 저장
- [x] round-trip: storage v1 entry → rehydrate 후 메모리 반영

**손상 캐시 — INITIAL fallback (silent fail 금지):**

- [x] 잘못된 JSON → INITIAL
- [x] choices 누락 → INITIAL
- [x] 알 수 없는 kind → INITIAL
- [x] preset 의 school 누락 → INITIAL
- [x] custom 의 annual 음수/0 → INITIAL
- [x] custom 의 annual NaN/string → INITIAL

**`resolveTuitionChoice` (순수 함수):**

- [x] choice 미지정 + entries 있음 → entries[0] fallback
- [x] preset 매칭 성공 → 그 entry 반환
- [x] preset 매칭 실패 (학교 사라짐) → entries[0] fallback
- [x] custom → entries 무시하고 `{ school: '직접 입력', annual, isCustom: true }`
- [x] entries 부재 + preset → null
- [x] entries 부재 + custom → custom 그대로 (사용자 입력 우선)
- [x] entries 부재 + choice 미지정 → null

**Hydration:**

- [x] `waitForAllStoresHydrated` 가 본 store 도 동시 await (7 store)
- [x] `forceInitialOnUnhydratedStores` 가 본 store 도 INITIAL 적용 분기 cover

### 9.8.3 `src/store/taxChoice.ts` (ADR-061)

세금 카테고리 단일 선택 + 직접 입력 store. 도시별 map. preset (annualSalary
매칭) 또는 custom (사용자 입력 annualSalary). custom 의 takeHomePctApprox 는 도시
첫 preset 의 값을 차용 — 단순화 정책 (세율은 도시별 평균값 사용).

**기본 동작:**

- [x] 초기 상태 `{ choices: {} }`
- [x] `setTaxChoice(cityId, preset)` / `setTaxChoice(cityId, custom)` 갱신
- [x] 도시별 독립
- [x] `clearTaxChoice` 미존재 cityId → no-op
- [x] `reset()` 전체 제거

**영속화:**

- [x] persist key 정확히 `taxChoice:v1`
- [x] partialize: 액션 미영속, choices 만
- [x] round-trip

**손상 캐시 — INITIAL fallback:**

- [x] 잘못된 JSON → INITIAL
- [x] 알 수 없는 kind → INITIAL
- [x] annualSalary 음수 → INITIAL

**`resolveTaxChoice` (순수 함수):**

- [x] choice 미지정 + entries → entries[0] fallback
- [x] preset 매칭 성공
- [x] preset 매칭 실패 → entries[0] fallback
- [x] custom → 사용자 annualSalary + 도시 첫 preset 의 takeHomePct
- [x] custom + entries 부재 → null (takeHomePct 차용 불가)
- [x] entries 부재 + 미지정 → null
...
- [x] 초기 상태 `{ inclusions: {} }`
- [x] `setInclusion(cityId, category, included)` 갱신
- [x] 같은 도시 내 여러 카테고리 독립 보존
- [x] 도시별 독립 (vancouver vs osaka)
- [x] setInclusion 같은 키 재설정 → 덮어쓰기
- [x] `resetCity(cityId)` → 그 도시만 제거
- [x] `resetCity` 미존재 cityId → no-op
- [x] `reset()` 전체 제거

**영속화:**

- [x] persist key 정확히 `categoryInclusion:v1`
- [x] partialize: 액션 미영속, inclusions 만
- [x] round-trip

**손상 캐시 — INITIAL fallback (silent fail 금지):**

- [x] 잘못된 JSON → INITIAL
- [x] inclusions 누락 → INITIAL
- [x] 알 수 없는 카테고리 키 → INITIAL
...
- [x] 26 IconName (`home, compare, star, settings, search, back, more, house, fork, bus, passport, graduation, briefcase, globe, chev-right, chev-down, info, refresh, mail, shield, book, user, plus, filter, up, close`) 각각 렌더
- [x] ICON_NAMES 배열은 26개 + 중복 없음
...
- [x] **centerMult undefined → 가운데 mult 미렌더 + caption 만 표시 (ADR-062)** — Compare 의 서울합=0 케이스에서 division by zero / `↑∞×` 회피
- [x] **centerMult + centerCaption 둘 다 undefined → 가운데 컬럼 자체 미렌더** (시각 균형)
- [x] **centerMult undefined + centerCaption 있음 → 가운데 컬럼 렌더 (caption 만 노출)**
...
**inclusion 토글 (ADR-062):**

- [x] `included` 기본 true + `onToggleInclude` 미지정 → 토글 미렌더, opacity 1, "제외됨" 배지 미렌더 (구 호출처 호환)
- [x] `onToggleInclude` 정의 + `included=true` → 토글 렌더 ON, 배지 미렌더, opacity 1
- [x] `onToggleInclude` 정의 + `included=false` → 토글 렌더 OFF, "제외됨" 배지 + opacity = `EXCLUDED_CARD_OPACITY` (0.55) — 색상에만 의존하지 않는 정보 표기 정책 (CLAUDE.md): opacity + 배지 + 토글 OFF 색 = 3중 인코딩
- [x] 토글 ON → OFF 탭 → `onToggleInclude(false)` 호출
- [x] 토글 OFF → ON 탭 → `onToggleInclude(true)` 호출
...
**onPress (단일 선택 모드 — detail rent):**

- [x] onPress 미지정 → 일반 View 로 렌더 (accessibilityRole 없음)
- [x] onPress 지정 → Pressable + accessibilityRole='button' + 탭 시 콜백 발화
- [x] onPress + selected=true → accessibilityState.selected=true

**selected 시각 표현 (사용자 피드백 2026-05-06 — strip 제거 + brand orange invert + 좌우 채움):**

- [x] selected=false (기본) → 행 배경 bg-orange 미적용
- [x] strip 은 더 이상 렌더되지 않는다 (좌측 검은 바 제거)
- [x] selected=true → 행 배경 bg-orange (brand) + 카드 폭 끝까지 채우는 px-3
- [x] selected=true → 텍스트 색 white 반전 (품목명 / 가격 / 배수 모두)
- [x] selected=true → emoji box white 로 invert (orange 배경 위 묻힘 방지)
- [x] selected=true + hot 행 → emoji box white (selected 가 hot 시각보다 우선)
- [x] selected=false + hot 행 → 기존 시각 (emoji orange-soft, mult orange) 유지

**행 padding 위치 (px-3 내부):**

- [x] px-3 가 행 className 에 항상 포함 (selected 무관) — 호출부의 wrapping padding 제거
...
### 9.20.4 `src/components/BottomSheet.tsx` (ADR-061)

RN Modal 기반 바텀시트 — top corners radius 22, white bg, navy text. 외부 영역
탭으로 dismiss + Android 백버튼 (onRequestClose). KeyboardAvoidingView 로 입력
포함 시트의 키보드 가림 회피.

- [x] visible=false → children 미렌더
- [x] visible=true → title + children 렌더
- [x] backdrop 탭 → onDismiss 호출 1회

### 9.20.5 `src/components/TuitionChoiceSheet.tsx` (ADR-061)

학교 선택 시트. 도시의 등록된 학교 목록 + 직접 입력 행 + 입력 모드 전환.
useTuitionChoiceStore 와 양방향 — 선택값에 따라 시트 진입 모드와 강조 결정.

**list 모드:**

- [x] 학교 목록 + 직접 입력 행 렌더
- [x] preset 탭 → store 갱신 + onDismiss 호출
- [x] 현재 선택이 preset 인 학교 → bg-orange 강조

**custom 모드:**

- [x] 직접 입력 행 탭 → custom 모드 (input + save + back 버튼)
- [x] custom 저장 → store 에 custom kind + onDismiss
- [x] 0 / 음수 / 비숫자 / 빈 입력 → save 무시 (no-op)
- [x] ← 학교 목록 탭 → list 모드 복귀
- [x] 현재 선택이 custom → 시트가 처음부터 custom 모드로 진입
- [x] 초기화 버튼 → store 에서 도시 entry 제거 + list 모드 복귀
...
연봉 기준 선택 시트. tuition sheet 와 구조 동일 — 연봉 tier 목록 + 직접 입력.

- [x] tier 목록 + 직접 입력 행 렌더
- [x] preset 탭 → store 갱신 + dismiss
- [x] custom 저장 → store 에 custom kind
- [x] 0 / 음수 입력 → save 무시
...
### 9.23 `app/(tabs)/index.tsx` (홈) — placeholder spec (deprecated)

screens phase step 2 에서 본 화면이 실제 구현됐고 테스트 인벤토리도 **§9.26** 으로 이전됐다.
검색 기능 (debounce·한글/영문 매칭) 은 v1.0 에서 stub 으로만 노출되며 (PRD 결정), 실제 구현은 v1.x 후속 phase 에서 다룬다.
...
**rent — useRentChoiceStore 연동 (ADR-060):**

- [x] 기본 'share' → 월세 카드 = 35만원 / 93.1만원 (vancouver fixture)
- [x] store='oneBed' → 월세 카드 = 120만원 / 225.4만원 (Detail 의 변경이 Compare 에 반영)
- [x] 도시에 선택 키 결측 → `resolveRentChoice` fallback (share → studio → oneBed → twoBed)
- [x] city.oneBed=null + store=oneBed → 양쪽 모두 share 기준 (서울 oneBed 사용 안 함, PR #24 review 이슈 2)
- [x] 마운트된 상태에서 store rentChoice 변경 → Compare 월세 카드 실시간 갱신 (PR #24 review 이슈 3)

**tuition / tax — useTuitionChoiceStore / useTaxChoiceStore 연동 (ADR-061):**

- [x] tuition 미선택 → 첫 entry (UBC) 기준 카드 (vancouver fixture, 367.5만원)
- [x] tuition store SFU preset → SFU 기준 (285.8만원)
- [x] tuition custom 18000 CAD/year → 월 1500 CAD = 147만원
- [x] tuition 마운트 후 store 갱신 → 카드 실시간 갱신
- [x] tax 미선택 → 첫 tier (60000) 기준 카드 mount
...
**rent 카테고리 (단일 선택 모드 — 합산 의미 없어 "선택된 행 1 개 기준" 비교, ADR-060):**

- [x] navy hero: 선택된 행 좌·우값 + caption "{카테고리} · {행 이름}"
- [x] 기본 선택 = 'share' (셰어하우스) — hero 좌·우값이 share 값과 일치
- [x] hero footer = "선택한 항목 기준 (탭으로 변경)"
- [x] selected 시각 (bg-orange) 은 기본 share 행에만 적용 (사용자 피드백 2026-05-06)
- [x] 다른 행 (oneBed) 탭 → hero 값/캡션 갱신 + selected 강조 이동
- [x] rent 행은 Pressable (accessibilityRole='button')
...
- [x] store 에 oneBed 가 미리 박혀 있으면 hero 가 oneBed 기준으로 mount (영속화)
- [x] 탭으로 바꾼 선택은 useRentChoiceStore 에 반영된다 (Compare 화면 연동의 단일 출처)
- [x] store=oneBed + city.oneBed=null → resolveRentChoice fallback 으로 share 선택 강조 (PR #24 review 이슈 1 — Compare 와 동일 정책)
- [ ] 페르소나 기반 default 선택 (학생: share, 직장인: studio 등) — v1.x 페르소나 분기 후속

**합계 모드 (rent 외 카테고리):**

- [x] food — hero footer "항목 단가 합" (기존 합계 모드 유지)
- [x] food — selected 강조 (bg-orange) 미적용 + 행 일반 View (selectable 아님)
...
**tuition 카테고리 (단일 선택 + 직접 입력 — ADR-061):**

- [x] navy hero: 선택된 학교 단가 (서울 0원 정책 — 한국 거주 기준)
- [x] "학교 (월 환산)" 섹션 — 1 row (현재 선택)
- [x] 미선택 → 첫 entry fallback (vancouver 의 UBC)
- [x] preset 변경 (store) → 다른 학교 row 로 갱신
- [x] custom 입력값 적용 → row 가 "직접 입력" 으로 표시
- [x] row 탭 → 시트 visible (`detail-tuition-sheet-body`)
...
- [x] navy hero: 선택된 연봉 tier 의 월 세금 (서울 0원)
- [x] "월 세금 (대략)" 섹션 — 1 row (현재 선택)
- [x] 미선택 → 첫 tier fallback (vancouver 의 60000)
- [x] preset 변경 → 다른 tier row 로 갱신
- [x] custom 입력값 → row 가 "(직접 입력)" 라벨
- [x] row 탭 → 세금 시트 visible
- [x] 도시에 tax 부재 → emptyText "세금 데이터가 아직 준비되지 않았어요."
...
- [x] 검색바 + TextInput 표시 (필터 아이콘 제거, 한글/영어 검색)
- [x] 아바타 버튼 표시 (screens step 2)

**도시 검색 (PRD F2 "도시 검색"):**

- [x] 검색 input 비어있을 때 즐겨찾기/최근/권역 섹션 노출
- [x] 한글 입력 시 부분 일치 결과만 노출 + 다른 섹션은 숨김
- [x] 영어 입력 (대소문자 무시) 부분 일치
- [x] 검색 결과 0건 — 빈 상태 메시지 ("검색 결과가 없어요")
- [x] 검색 결과 행 탭 → `/compare/{cityId}` push
- [x] 서울은 검색 결과에서 제외 (비교 대상 아님)
- [x] clear 버튼 → query 초기화 + 원래 섹션 복귀
- [x] 공백만 입력 시 검색하지 않음 (원래 섹션 유지)
...
- [x] RegionPill 6개 렌더링 (전체/북미/유럽/아시아/오세아니아/중동) (screens step 2)
- [x] 기본 active = 전체 (screens step 2)
- [x] 권역 탭 시 active 토글 (screens step 2)
- [x] 전체 선택 시 모든 해외 도시 노출 (서울 제외, 가나다 순)
- [x] 권역 선택 시 해당 권역 도시만 노출
- [x] 권역 도시 행 탭 → `/compare/{cityId}` push

**설정 진입:**

- [x] 아바타 탭 → `/settings` navigate (push 아님 — 탭 stack 누적 회피, PR #18 review round 8)

**에러 상태:**

- [x] 서울 데이터 없음: 에러 메시지 표시 (screens step 2)
- [x] 다시 시도 버튼 → loading 상태로 전환 후 재로드 (PR #18 review round 6)
- [ ] 네트워크 오류: ErrorView 패턴 (Compare 화면처럼)
...
§6.6 "스냅샷 100라인 정책" 준수용. `render(...).toJSON()` 결과에서 `testID` 일치 서브트리만 추출. 화면 전체 대신 핵심 영역만 캡처해 시각 회귀 검출 + 가독성 양립.
