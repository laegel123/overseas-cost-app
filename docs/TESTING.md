...
- [ ] persona.onboarded=false → `/onboarding` redirect (app-shell step 2)
- [ ] persona.onboarded=true → `(tabs)` 렌더 (app-shell step 2)
- [ ] 에러 boundary: 자식 트리 throw → ErrorView (fatal) (app-shell step 3)
...
- [ ] hydration timeout (5s) → INITIAL_STATE fallback + warn (app-shell step 1, ADR-052)
