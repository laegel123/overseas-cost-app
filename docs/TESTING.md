...
- **계약 우선.** 함수 시그니처·컴포넌트 prop 의 *의미* 를 테스트로 고정 (구현 변경에 흔들리지 않게).
...
export const buildCity = (overrides?: DeepPartial<CityCostData>): CityCostData => ({
  ...
  sources: [{ category: 'rent', name: 'Kijiji 평균', url: 'https://...', accessedAt: '2026-04-01' }],
  ...deepMerge(overrides ?? {}),
});
```
...
export function expectComparePair(rendered, expected: { mult: string; sw: number; cw: number; hot?: boolean }) {
...
#### `validateAllJson(json): { schemaVersion, cities, generatedAt, fxBaseDate }` — 배치 파일 검증

- [ ] schemaVersion === 1
- [ ] cities 가 객체
- [ ] cities 의 키 ≥ 1
- [ ] cities 의 모든 값이 validateCity 통과
- [ ] generatedAt 이 ISO 형식
- [ ] 실패 시 throws `DataSchemaVersionError` 또는 `DataParseError`
...
## 9-A. 자동화 스크립트 (scripts/refresh/* + scripts/build/*)
