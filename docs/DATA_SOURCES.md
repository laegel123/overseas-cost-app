...
> **`sources[].url` 은 도시별 실제 공공 출처를 가리킨다 (ADR-071).** `universities.mjs` / `visas.mjs` 는 모듈 상수 `SOURCE` 대신 `buildSource(cityId)` 로 도시별 디스크립터를 만든다 — 학비는 위 각 도시 §학비 절의 **첫 번째 대학 URL**, 비자는 §비자 절의 **정부 공식 페이지 URL** 이 그대로 기록되고, 출처명도 `UBC · SFU · BCIT 공식 국제학생 학비 페이지 (정적 추정치)` / `캐나다 이민·난민·시민권부(IRCC) 공식 비자 수수료 페이지 (정적 추정치)` 처럼 도시별로 구체화된다. 이전에는 두 카테고리 40개 출처가 모두 본 문서의 GitHub URL 을 가리켰다. 도시별 학비·비자 URL 을 수정할 때는 본 문서와 `scripts/refresh/{universities,visas}.mjs` 의 registry 를 함께 갱신한다 (registry 가 코드상 단일 출처).
