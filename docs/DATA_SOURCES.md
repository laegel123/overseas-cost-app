...
> **출처 유형 총수: 12** — 정부 통계 / 공식 교통공사 / 공식 대학 페이지 / 환율 API 등 카테고리별 활용 출처. Settings 화면의 `DATA_SOURCES_COUNT` 와 동기화 필수 (`app/(tabs)/settings.tsx`). 출처 추가 시 본 라인과 코드 상수 동시 갱신. v1.x 빌드 타임 자동 카운트로 전환 예정.
